...
class LambdaRuntime
{
    ...
    {
        $result = $this->curl("/2018-06-01/runtime/invocation/" . $this->getRequestId() . "/response",
            LambdaRuntime::POST, $this->getResponse());
        $this->resetResponse();
    }
    ...
    public function getNextEventData()
    {
        ...
        if (!isset($this->rawEventData["headers"]["lambda-runtime-aws-request-id"][0])) {
            //Handle error
            $this->reportError("MissingEventData",
    ...
    public function reportError($errorType, $errorMessage)
    {
        ...
        $result = $this->curl("/2018-06-01/runtime/invocation/" . $this->getRequestId() . "/error", LambdaRuntime::POST,
            $errorPayload);
    }
    ...
    public function reportInitError($errorType, $errorMessage)
    {
        $errorArray = array("errorType" => $errorType, "errorMessage" => $errorMessage);
        $errorPayload = json_encode($errorArray);
        $result = $this->curl("/2018-06-01/runtime/init/error", LambdaRuntime::POST, $errorPayload);
    }
...
while (true) {
    ...
    try {
        ...
        //Execute handler
        $functionReturn = handler($eventPayload);
